...
def compute_mfcc(fs_hz, signal, signal_length, ref, cmnor = True):
	'''
	Args:
	...
	plt.savefig('C:/Users/dell/Desktop/assignments/ST/extra_task/melspec_and_power_spectrogram.png')
	...
	if cmnor == True:
		...
		plt.xlabel ('Time')
		plt.colorbar ()
		plt.savefig ('C:/Users/dell/Desktop/assignments/ST/extra_task/feat_mfcc_z.png')
	else:
		plt.figure (2)
		plt.subplot (211)
		plt.imshow (feat_mfcc.T, origin='lower', aspect='auto', extent=(0, siglen, 1, num_ceps), cmap='jet')
		...
		plt.xlabel ('Time')
		plt.colorbar ()
		plt.savefig ('C:/Users/dell/Desktop/assignments/ST/extra_task/feat_mfcc.png')
	# show the reference pic
	plt.figure(3)
	plt.subplot(211)
	plt.imshow(ref.T, origin='lower', aspect='auto', extent=(0, siglen, 1, num_ceps), cmap='jet')
	plt.title('reference')
	plt.ylabel('MFCC coeffcients')
	plt.xlabel('Time')
	plt.colorbar()
	plt.savefig ('C:/Users/dell/Desktop/assignments/ST/extra_task/reference.png')





	# Mapping the sound spectrum
	plt.figure (4)
	plt.subplot (311)
	plt.specgram (signal, Fs=fs_hz, cmap='jet')
	plt.xlabel ('Time')
	...
	mfcc_feat = (mfcc_feat - np.mean (mfcc_feat)) / np.std (mfcc_feat)
	mfcc_feat = np.append (mfcc_feat[1:], mfcc_feat[-1:])

	delta = sig.convolve(mfcc_feat, [-1, 0, 1], mode='same')
	delta_delta = sig.convolve(delta, [-1, 0, 1], mode='same')

	# Plotting first-order differential coefficient images
	plt.subplot (312)
	plt.plot (delta)
	plt.xlabel ('Frame')
	plt.ylabel ('Delta')
	plt.title ('Delta Coefficients')
	plt.subplots_adjust (hspace=0.8)
	...
	plt.title ('Delta-Delta Coefficients')
	plt.savefig('C:/Users/dell/Desktop/assignments/ST/extra_task/12jie.png')
	plt.show()
...
compute_mfcc(fs_hz, signal, signal_length, ref, cmnor=True)
