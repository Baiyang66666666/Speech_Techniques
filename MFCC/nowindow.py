...
def compute_mfcc(fs_hz, signal, signal_length, ref, configs, cmnor = True):
	...
	# mel_filters = com_mel_filters(num_filters, fs_hz, mel_filters, NFFT)
	low_freq_mel = 0
	...
	# Hamming window, change to no window,This will remove the Hamming window, and instead apply a rectangular window to each frame.
	...
	if cmnor == False:
		...
		# Save MFCC to text file
		filename = "feat_mfcc"
		save_mfcc(feat_mfcc, filename)
	else:
		# Cepstral mean and variance normalisation
		feat_mfcc_z = mfcc
		feat_mfcc_z -= (np.mean (mfcc, axis=0) + 1e-8)
		...
		save_mfcc(feat_mfcc_z, filename)
	...
	plt.savefig(f'C:/Users/dell/Desktop/assignments/ST/configs/{configs}_signal_and_power_spectrogram.png')
	if cmnor == True:
		...
		plt.xlabel ('Time')
		plt.colorbar ()
		plt.savefig (f'C:/Users/dell/Desktop/assignments/ST/configs/{configs}_feat_mfcc.png')
	plt.show()
